...
| `src/components/` | 재사용 UI·조각. `ui/`(shadcn 프리미티브), `scaffold/`(페이지 뼈대·스켈레톤), `forms/`, `posts/`, `error-boundary/`, `site-header`·`require-auth` 등. |
| `src/hooks/` | 클라이언트 전용 React 훅(예: 무한 스크롤용 `IntersectionObserver`). |
| `src/lib/` | 앱 전역 유틸·설정: `env`, NextAuth `auth-options`, 클라이언트용 `auth-api`·`http-request-log`, `flow-log`, `query-cache`, `trpc-server-access-log`(서버에서만 import — 터미널 tRPC 풀이 로그) 등. |
| `src/server/` | **서버 전용** 도메인: `db/`(Drizzle 스키마·연결), `services/`(비즈니스 로직), `trpc/`(라우터·프로시저·컨텍스트), `upload/`(이미지 저장 헬퍼). 클라이언트 컴포넌트에서 직접 import하지 않는다. |
| `src/trpc/` | 브라우저/RSC에서 쓰는 **tRPC 클라이언트** 조립: `TrpcProvider`, `httpBatchLink`, 세션 쿠키 포함 `fetch`. |
...
| `app/login`, `register`, `posts`, `profile` | 각 화면 라우트. 동적 세그먼트는 `posts/[id]` 등. |
...
| 데이터 UI | `post-list-client.tsx`, `post-detail-client.tsx`, `post-edit-client.tsx` | tRPC React 훅, `useSession`; 목록은 `useIntersectionInfiniteScroll` 로 스크롤 감지 분리 |
...
| **Zod 4** | 이메일은 `z.email()`, UUID는 `z.uuid()` 등 [문자열 체인 대신 포맷 스키마](https://zod.dev)를 쓰는 방향으로 맞춰 두었다. |
...
| **낙관적 업데이트(Optimistic UI)** | 글 수정·삭제 직후 서버 응답 전에 UI를 먼저 바꾸고, 실패 시 롤백. TanStack Query `onMutate` / `placeholderData` 흐름. |
| **RSC에서 데이터 직접 로드** | 지금은 목록 “살”이 클라이언트 tRPC다. `appRouter.createCaller` 또는 서비스 레이어를 RSC에서 호출해 **첫 HTML에 목록 포함**시키는 실험을 해 보면 SSR/RSC 이해가 깊어진다. |
...
| **DB·배포** | Drizzle **마이그레이션 파일** 기반 배포(`db:generate` → CI 적용), 연결 풀(`postgres` 옵션), 읽기 복제 등. |
